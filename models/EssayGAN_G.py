...
class EssayGAN_G(LSTMGenerator):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, max_seq_len, padding_idx, gpu=False, loss_type="sentigan"):
        super(EssayGAN_G, self).__init__(embedding_dim, hidden_dim, vocab_size, max_seq_len, padding_idx, gpu)
        self.name = 'sentigan'
    ...
    def batchPGLoss(self, inp, target, reward):
        ...
        if(self.loss_type == "sentigan"):
            out = self.forward(inp, hidden, use_log=False).view(batch_size, self.max_seq_len, self.vocab_size) # sentigan
        elif(self.loss_type == "seqgan"):
            out = self.forward(inp, hidden, use_log=True).view(batch_size, self.max_seq_len, self.vocab_size) # seqgan
        ...
        if (self.loss_type == "sentigan"):
            loss = -torch.sum(pred * (1 - reward)) # sentigan code
            # loss = torch.sum(pred * (1 - reward)) # sentigan paper
        elif (self.loss_type == "seqgan"):
            loss = -torch.sum(pred * reward) # seqgan
